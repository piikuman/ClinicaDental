...
BEGIN
  INSERT INTO DOCTORA(NOMBRE, APELLIDOS, DNI, FECHA_NACIMIENTO,POBLACION,DIRECCION,FECHAALTA,TELEFONO,SUELDO,CODIGODOCTORA)
  VALUES (P_NOM ,P_APE,P_DNI,P_FECHA_NAC,P_POBLACION,P_DIRECCION,P_FECHAALTA,P_TEL,P_SUELDO,P_CODIGODOCTORA);
END;
/
...
BEGIN
    UPDATE Doctora
    SET NOMBRE=P_NOM, APELLIDOS=P_APE, DNI=P_DNI, FECHA_NACIMIENTO=P_FECHA_NAC,POBLACION=P_POBLACION,DIRECCION=P_DIRECCION,FECHAALTA=P_FECHAALTA,TELEFONO=P_TEL,SUELDO=P_SUELDO
    WHERE CODIGODOCTORA=P_CODIGODOCTORA;
END;
/
...
CREATE OR REPLACE PROCEDURE ELIMINAR_DOCTORA(CODIGO_DOCTORA_A_QUITAR IN doctora.codigodoctora%TYPE) AS
BEGIN
  DELETE FROM DOCTORA WHERE codigodoctora = CODIGO_DOCTORA_A_QUITAR;
END;
/
